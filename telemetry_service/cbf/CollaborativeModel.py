from cbf import GlocalArchitecture
...
class Glocal:
    ...
    def finetune(self,train_r, train_m):
        ...
        for i in range(self.max_epoch_f):
            ...
            self.pre, _, _ = self.complete_net(torch.Tensor(train_r).double().to(self.device), torch.Tensor(train_r_local).double().to(self.device))
